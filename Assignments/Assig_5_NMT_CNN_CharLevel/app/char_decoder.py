...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        if VERBOSE: print('Input embedded: ', X.shape)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        if VERBOSE: print('char_sequence: ', char_sequence)
        ...
        if VERBOSE: print('scores view :', scores.shape)
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        if VERBOSE: print('Batch size = ', BS)
        
        output_word = []
        decoded_words = ['{'] * BS
        current_char = [self.target_vocab.char2id['{']] * BS                                    # ()

        if VERBOSE: print('Current char [0]: {} :: {}'.format(current_char[0], self.target_vocab.id2char[current_char[0]]))
        ...
        for j,t in enumerate(range(max_length)):
            ...
            if j == 0 and VERBOSE: print('Current char tensor: ', current_char_tensor.shape)
            if j == 0 and VERBOSE: print('Current char tensor unsqueezed shape: ', current_char_tensor.unsqueeze(0).shape)   # ()
            _, (h_t,c_t) = self.forward(current_char_tensor.unsqueeze(0), (h_prev, c_prev))     # ()
            
            # Compute Scores
            if j == 0 and VERBOSE: print('Hidden shape: ', h_t.shape)                    # ()
            if j == 0 and VERBOSE: print('Hidden squeezed shape: ', h_t.squeeze(0).shape)                    # ()
            scores = self.char_output_projection(h_t.squeeze(0))                                # ()
            if j == 0 and VERBOSE: print('scores: ', scores)
            if j == 0 and VERBOSE: print('scores shape: ', scores.shape)
            
            # Compute Probabilities
            P = F.log_softmax(scores, dim=1)                                                    # ()
            if j == 0 and VERBOSE: print('probabilities: ', P)
            if j == 0 and VERBOSE: print('probabilities shape: ', P.shape)
            
            # Compute predicted character  
            current_char_tensor = torch.argmax(P, dim=1)                                               # ()
            if j == 0 and VERBOSE: print('predicted_char: ', current_char_tensor)
            if j == 0 and VERBOSE: print('predicted_char shape: ', current_char_tensor.shape)
        ...
        if VERBOSE: print('Decoded words: ', decoded_words)
        for i in range(BS):
            # Remove start token
